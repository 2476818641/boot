...
📡 **校园网 / UA 伪装说明 →** [Docs/CAMPUS-SETUP.md](Docs/CAMPUS-SETUP.md)
（本 fork 加了 UA-Mask：把各设备 UA 统一成一台 PC，用于规避校园网「多设备共享」检测。
含：为什么不用 UA3F、刷完怎么一键配好校园网登录、以及高通 NSS 硬件加速与 TTL 改写的注意点）
