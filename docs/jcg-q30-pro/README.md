...
| [RECOVERY-SLIM-PLAN.md](RECOVERY-SLIM-PLAN.md) | 方案 | 精简 recovery 镜像（29MB → 9.0MB，解包 95MB → 20.5MB）：**已集成进默认构建**，含包清单、构建脚本、失败记录与实测数据 |
...
- 「按住 reset」救砖靠**精简 recovery**（9.0MB）—— **默认构建产物就是它**，
  一条命令搞定：`bash scripts/build-recovery-slim.sh`
